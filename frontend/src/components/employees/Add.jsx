...
import { 
  ...
  UserPlus, Save, Phone, Users, IdCard, AlertCircle
} from 'lucide-react';
...
const AddEmployee = () => {
  ...
  const [formData, setFormData] = useState({
    name: '',
    email: '',
    // employeeId: '',  // <-- removed, backend will generate
    dob: '',
    gender: '',
  ...
  const calculatePAYE = (taxable) => {
    ...
    if (annual <= 288000) {
      ...
      tax = 288000 * 0.1 + (annual - 288000) * 0.25;
    } else if (annual <= 6000000) {
      tax = 288000 * 0.1 + 100000 * 0.25 + (annual - 388000) * 0.3;
    } else if (annual <= 9600000) {
      tax =
        288000 * 0.1 +
        100000 * 0.25 +
        5612000 * 0.3 +
        (annual - 6000000) * 0.325;
    } else {
      tax =
        288000 * 0.1 +
        100000 * 0.25 +
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // append all fields except employeeId (we removed it from state anyway)
      ...
      if (response.data.success) {
        const generatedId = response.data.employeeId; // we added this in backend
        alert(
          `Employee added successfully! Generated Employee ID: ${generatedId || '—'}`
        );
        clearForm();
      ...
      const errorMessage =
        error.response?.data?.error || error.message || 'Failed to add employee';
            ...
            <div className="flex items-center gap-6">
              ...
              <div>
                ...
                <p className="text-white/90">
                  ...
                  and system-generated employee code.
                </p>
              </div>
            </div>
          </div>
            ...
            <div className="flex items-center justify-between mb-6">
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-gray-600 text-sm">
                    Automatic KRA tax calculations
                  </p>
                </div>
              </div>
              {salaryBreakdown && (
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between">
                        ...
                        <span className="font-semibold">
                          KES {salaryBreakdown.basic.toLocaleString()}
                        </span>
                      </div>
                      {salaryBreakdown.allowances.housing > 0 && (
                        <div className="flex justify-between text-gray-600">
                          ...
                          <span>
                            KES{' '}
                            {salaryBreakdown.allowances.housing.toLocaleString()}
                          </span>
                        </div>
                      )}
                      {salaryBreakdown.allowances.transport > 0 && (
                        <div className="flex justify-between text-gray-600">
                          ...
                          <span>
                            KES{' '}
                            {salaryBreakdown.allowances.transport.toLocaleString()}
                          </span>
                        </div>
                      )}
                      {salaryBreakdown.allowances.medical > 0 && (
                        <div className="flex justify-between text-gray-600">
                          ...
                          <span>
                            KES{' '}
                            {salaryBreakdown.allowances.medical.toLocaleString()}
                          </span>
                        </div>
                      )}
                      {salaryBreakdown.allowances.other > 0 && (
                        <div className="flex justify-between text-gray-600">
                          ...
                          <span>
                            KES{' '}
                            {salaryBreakdown.allowances.other.toLocaleString()}
                          </span>
                        </div>
                      )}
                      <div className="flex justify-between pt-2 border-t border-green-200 font-bold text-green-700">
                        ...
                        <span>
                          KES {salaryBreakdown.gross.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="space-y-1 text-sm">
                      <div className="flex justify-between text-gray-600">
                        <span>NHIF:</span>
                        <span>
                          KES {salaryBreakdown.deductions.nhif.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-gray-600">
                        <span>NSSF:</span>
                        <span>
                          KES {salaryBreakdown.deductions.nssf.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-gray-600">
                        ...
                        <span>
                          KES{' '}
                          {salaryBreakdown.deductions.housingLevy.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between text-gray-600">
                        <span>PAYE (Tax):</span>
                        <span>
                          KES {salaryBreakdown.deductions.paye.toLocaleString()}
                        </span>
                      </div>
                      <div className="flex justify-between pt-2 border-t border-red-200 font-bold text-red-700">
                        <span>Total Deductions:</span>
                        <span>
                          KES {salaryBreakdown.totalDeductions.toLocaleString()}
                        </span>
                      </div>
                    </div>
                  </div>
                ...
                <div className="mt-4 p-3 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-start gap-2">
                    ...
                    <p className="text-xs text-blue-800">
                      This is a preview calculation. Final amounts will be
                      calculated by the server using official KRA tax rates for
                      2024/2025.
                    </p>
                  </div>
                </div>
              ...
              >
                {submitting ? (
                  ...
                  <>
                    ...
                    <span>Add Employee with Auto-ID</span>
                  </>
                )}
              </button>
            </div>
